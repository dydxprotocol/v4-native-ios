...
class CombineObservingTests: XCTestCase {
    ...
    func disabled_testCombineObservingPerformance() throws {
        measure {
            ...
            observer.initialBlock = { val, animated in
                receivedInitialVals.append(val)
            }
            
            observer.changeBlock = { val, animated in
                receivedChangeVals.append(val)
            }
...
fileprivate class PublisherMock {
    @Published var count: Int = 0
}

fileprivate class ObserverMock: CombineObserving {
    var cancellableMap = [AnyKeyPath : AnyCancellable]()
    
    var initialBlock: ((_ obj: Int?, _ emitState: EmitState) -> ()) = { _, _ in }
    var changeBlock: ((_ obj: Int?, _  emitState: EmitState) -> ()) = { _, _ in }
