...
extension String {
    public func color() -> String? {
        let hash = abs(sdbmhash)
        let colorNum = hash % (256 * 256 * 256)
        let red = ((colorNum >> 16) * 4 / 5)
        let green = (((colorNum & 0x00FF00) >> 8) * 4 / 5)
        let blue = (colorNum & 0x0000FF) * 4 / 5
        let redHex = String(format: "%X", red)
        let greenHex = String(format: "%X", green)
        let blueHex = String(format: "%X", blue)
