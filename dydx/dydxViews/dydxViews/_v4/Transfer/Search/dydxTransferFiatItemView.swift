        ...
        PlatformView(viewModel: self, parentStyle: parentStyle, styleKey: styleKey) { [weak self] style in
            ...
            let view = HStack(spacing: 8) {
                ...
                VStack(alignment: .leading) {
                    HStack {
                        Text(DataLocalizer.localize(path: "APP.ONBOARDING.CASH_WITH",
                                                    params: ["{VENDER}": "MoonPay"]))
                        .themeFont(fontSize: .medium)
                        .themeColor(foreground: .textPrimary)

                        PlatformIconViewModel(type: .asset(name: "logo_moonpay", bundle: Bundle.dydxView),
                                                  size: CGSize(width: 16, height: 16))
                                .createView(parentStyle: style)
                    }
                    Text("ApplePay, PayPal, " + DataLocalizer.localize(path: "APP.ONBOARDING.DEBIT"))
