    ...
    private func deposit(selectedRoute: TransferRouteSelection?) {
        Publishers.Zip(AbacusStateManager.shared.state.transferInput,
            ...
            .flatMapLatest { [weak self] input, wallet in
                ...
                Tracking.shared?.log(event: AnalyticsEventV2.DepositInitiatedEvent(transferInput: input,
                                                                                   summary: summary))
            ...
            .sink { [weak self] event, transferInput in
                ...
                case let .result(hash, error):
                    if let error = error {
                        Tracking.shared?.log(event: AnalyticsEventV2.DepositErrorEvent(transferInput: transferInput,
                        ...
                        Tracking.shared?.log(event: AnalyticsEventV2.DepositSubmittedEvent(transferInput: transferInput,
                        ...
                        Tracking.shared?.log(event: AnalyticsEventV2.DepositErrorEvent(transferInput: transferInput, errorMessage: "No hash"))
    ...
    private func withdrawal() {
        ...
        .sink { [weak self] transferInput, currentWallet, selectedSubaccount, usdcBalanceInWallet in
            ...
            self.screen(originationAddress: originationAddress, destinationAddress: destinationAddress) { success in
                ...
                Tracking.shared?.log(event: AnalyticsEventV2.WithdrawInitiatedEvent(transferInput: transferInput,
                                                                                   summary: transferInput.summary))
                ...
                if transferInput.isCctp {
                    AbacusStateManager.shared.commitCCTPWithdraw { [weak self] success, error, result in
                        if success {
                            ...
                            Tracking.shared?.log(event: AnalyticsEventV2.WithdrawErrorEvent(transferInput: transferInput, errorMessage: error?.localizedDescription ?? ""))
                    ...
                    if usdcBalanceInWallet >= gasFee {
                        ...
                        Tracking.shared?.log(event: AnalyticsEventV2.WithdrawErrorEvent(transferInput: transferInput, errorMessage: "No gas"))
        ...
        if let result = (result as? String)?.jsonDictionary {
            if let error = result["error"] as? [String: Any] {
                ...
                if transferInput.type == .withdrawal {
                    Tracking.shared?.log(event: AnalyticsEventV2.WithdrawErrorEvent(transferInput: transferInput, errorMessage: message ?? ""))
                }
                ErrorInfo.shared?.info(title: DataLocalizer.localize(path: "APP.GENERAL.ERROR"),
                                       message: message,
                                       type: .error,
                ...
                if transferInput.type == .withdrawal {
                    Tracking.shared?.log(event: AnalyticsEventV2.WithdrawSubmittedEvent(transferInput: transferInput,
                                                                                        summary: transferInput.summary,
                                                                                        txHash: hash,
                ...
                if transferInput.type == .withdrawal {
                    Tracking.shared?.log(event: AnalyticsEventV2.WithdrawErrorEvent(transferInput: transferInput, errorMessage: "No hash"))
                }
                ErrorInfo.shared?.info(title: DataLocalizer.localize(path: "APP.GENERAL.ERROR"),
                                       message: DataLocalizer.localize(path: "APP.V4.NO_HASH"),
                                       type: .error,
