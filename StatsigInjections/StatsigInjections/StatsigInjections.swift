        ...
        if Statsig.isInitialized() {
            ...
            Statsig.start(sdkKey: apiKey, user: StatsigUser(userID: userId), options: StatsigOptions(
                ...
                urlSession: nil)) {[weak self] error in
                    Console.shared.log("analytics log | Statsig finished remote fetch feature flags")
                    if let error {
                        Console.shared.log("analytics log | Statsig finished remote fetch feature flags with error \(error)")
                        return
                    }
...
//                    completion()
            ...
            Console.shared.log("analytics log | Statsig feature flags initialized (before remote fetch) | env : \(self.environment.debugDescription) | userId: \(userId)")
